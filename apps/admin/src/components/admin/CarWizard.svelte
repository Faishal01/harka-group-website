...
<div
	class="max-w-4xl mx-auto bg-white text-gray-900 rounded-xl shadow-md overflow-hidden border border-gray-200"
>
	<!-- Progress Header -->
		...
		<form on:submit|preventDefault={submitForm}>
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
					<div class="col-span-1 md:col-span-2">
						...
						<input
							...
							placeholder="e.g. 2026 Porsche 911 Turbo S"
							required
						/>
					</div>
					...
					>
						<div>
							...
							<p class="text-xs text-gray-500 mt-1 ml-8">
								If checked, this car will not appear on the main website until you edit it and
								unhide it.
							</p>
						</div>
					</div>
			...
			<div class={currentStep === 4 ? "block animate-fade-in" : "hidden"}>
				...
				<div class="grid grid-cols-1 gap-6">
					<div
						class="bg-gray-50 border border-dashed border-gray-300 hover:border-blue-500 rounded-lg p-8 text-center hover:border-blue-500 transition cursor-pointer relative"
					>
						<input
							type="file"
							bind:files
						...
						<p class="text-gray-400 font-medium">
							{files && files.length
								? files[0].name
								: "Drag and drop main image here, or click to browse"}
						</p>
					</div>
			...
			<div class="mt-10 flex items-center justify-between pt-6 border-t border-gray-200">
				<button
					type="button"
					class="px-6 py-2.5 rounded font-bold uppercase tracking-wider text-sm transition {currentStep ===
					1
				...
				{:else}
					<button
						type="submit"
						disabled={isLoading}
						class="bg-blue-600 hover:bg-blue-700 text-white px-8 py-2.5 rounded font-semibold transition disabled:opacity-50 flex items-center gap-2"
					>
						{#if isLoading}
							<svg
